...
```bash
NAMESPACE_NAME=perf-test-$RANDOM
RG=perf-rg
QUEUE_NAME=test-queue
az servicebus namespace create --name "$NAMESPACE_NAME" --resource-group "$RG" --location westeurope --sku Premium
az servicebus queue create --resource-group "$RG" --namespace-name "$NAMESPACE_NAME" --name "$QUEUE_NAME"
az role assignment create --assignee "$(az account show --query user.name -o tsv)" --role "Azure Service Bus Data Owner" --scope "$(az servicebus namespace show --resource-group "$RG" --name "$NAMESPACE_NAME" --query id -o tsv)"
```
